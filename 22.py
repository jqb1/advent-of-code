...
def move(move_cnt, direction, position, grid):
    ...
    for i in range(move_cnt):
        if pos_r + dr == max_r or pos_c + dc == max_c or pos_c + dc == -1 or pos_r + dr == -1 or grid[pos_r + dr][pos_c + dc] == ' ':
            new_pos_r, new_pos_c, new_direction = wrap_side((pos_r, pos_c), direction, grid)
